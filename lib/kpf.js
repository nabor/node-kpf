...
function FlowAPI(request, config) {
    ...
    */
    this.new_order = function (orden_compra, monto,  concepto, tipo_comision) {
        ...
        _flowLog("Iniciando nueva Orden", "new_order");
        if(!orden_compra || !monto || !concepto) {
            _flowLog("Error: No se pasaron todos los parámetros obligatorios","new_order");
        }
        if(typeof monto !== 'number') {
            _flowLog("Error: El parámetro monto de la orden debe ser numérico","new_order");
    ...
    *
    */
    this.read_confirm = function (response_string) {
        var response = urljson.decode(response_string);
        if(!response) {
            _flowLog("Respuesta Inválida", "read_confirm");
            throw new Error('Invalid response');
        }
        if(!response.status) {
            _flowLog("Respuesta sin status", "read_confirm");
            throw new Error('Invalid response status');
        }
        _order.status = response.status;
        _flowLog("Lee Status: " + response.status, "read_confirm");
        if (!response.s) {
            _flowLog("Mensaje no tiene firma", "read_confirm");
            throw new Error('Invalid response (no signature)');
        }
        if(!_flow_sign_validate(response.s, response_string)) {
            _flowLog("firma invalida", "read_confirm");
            throw new Error('Invalid signature from Flow' );
        }
        _flowLog("Firma verificada", "read_confirm");
        if(response.status == "ERROR") {
            _flowLog("Error: " + response.kpf_error, "read_confirm");
        ...
        _flowLog("Lee Numero Orden: " + response.kpf_orden, "read_confirm");
        if(!response.kpf_monto) {
            throw new Error('Invalid response Amount');
        }
        _order.monto = response.kpf_monto;
        _flowLog("Lee Monto: " + response.kpf_monto, "read_confirm");
        if(response.kpf_flow_order) {
            _order.flow_id = response.kpf_flow_order;
            _flowLog("Lee Orden Flow: " + response.kpf_flow_order, "read_confirm");
        }
        return _order;
    };
    ...
    */
    this.build_response = function (result){
        ...
        var s = _flow_sign(q);
        _flowLog("Orden N°: " + _order.id + " - Status: " + r,"flow_build_response");
        return q + "&s=" + s;
    };
    ...
    *
    */
    this.read_result = function (response_string) {
        var response = urljson.decode(response_string);
        if(!response) {
            _flowLog("Respuesta Inválida", "read_result");
            throw new Error('Invalid response');
        }
        if (!response.s) {
            _flowLog("Mensaje no tiene firma", "read_result");
            throw new Error('Invalid response (no signature)');
        }
        if(!_flow_sign_validate(response.s, response_string)) {
            _flowLog("firma invalida", "read_result");
            throw new Error('Invalid signature from Flow');
        }
        ...
        _flowLog("Datos recuperados Orden de Compra N°: " + response.kpf_orden, "read_result");
    };
